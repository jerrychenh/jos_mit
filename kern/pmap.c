...
mem_init(void)
{
	...
	// Your code goes here:
	size_t len = npages * sizeof(struct PageInfo);
	pages = boot_alloc(len);
	memset(pages, 0, len);
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, 0xffffffff - KERNBASE/*do not use 2^32, signed*/, 0, PTE_W | PTE_P);
	assert(kern_pgdir[0] == 0);
...
page_alloc(int alloc_flags)
{
	...
	if (alloc_flags & ALLOC_ZERO){
		// use page2kva because in lab1, we set the addr map KERNBASE -> 1M
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(((*pde_ptr) & PTE_P) == 0){
		...
		if(p != NULL){
			p->pp_ref++;
			*pde_ptr = page2pa(p) | PTE_P | PTE_U | PTE_W;
		}
		else{
			return NULL;
		}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while(pos < size){
		...
		if(pte == NULL){
			panic("error in boot_map_region");
		}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	// first inc, so that page_remove will not remove same page
	pp->pp_ref++;
	if(((*pte) & PTE_P) != 0){
